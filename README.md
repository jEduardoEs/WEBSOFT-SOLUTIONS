...
1. Descarga o clona el repositorio y asegúrate de descomprimirlo (si usaste un archivo `.zip`).
2. Abre una terminal en la carpeta `preview` y lanza un servidor local rápido:

   ```bash
   python -m http.server 8080
   ```

   > También puedes usar cualquier otra herramienta que sirva archivos estáticos (Live Server de VSCode, http-server, etc.).

3. Visita `http://localhost:8080/index.html` en tu navegador preferido. De esta forma los estilos `styles.css` se cargarán sin que el navegador los bloquee por políticas de seguridad del esquema `file://`.

Si aun así necesitas abrirlo directamente como archivo (`file://`), el documento mostrará un aviso recordándote que levantes el servidor para apreciar el diseño final.
